...
class KakaoAddressClient:
    def __init__(self, rest_api_key: str | None = None, timeout_seconds: float = 6.0, *, cache_ttl_seconds: float = 300, cache_max_entries: int = 256) -> None:
        self.rest_api_key = (rest_api_key or os.getenv("KAKAO_REST_API_KEY", "")).strip()
        self.timeout_seconds = timeout_seconds
        self.cache: TtlCache[list[AddressSearchItem]] = TtlCache(ttl_seconds=cache_ttl_seconds, max_entries=cache_max_entries)
    ...
    async def search_addresses(self, *, query: str, limit: int = 10) -> list[AddressSearchItem]:
        if not self.rest_api_key:
            raise KakaoAddressUnavailable("KAKAO_REST_API_KEY가 설정되지 않았습니다.", reason="not_configured")
        cache_key = json.dumps([query.strip(), limit], ensure_ascii=False, separators=(",", ":"))
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout_seconds) as client:
                ...
                if response.status_code == 429:
                    raise KakaoAddressUnavailable("Kakao Local 호출 한도에 도달했습니다.", reason="quota")
                if response.status_code in {401, 403}:
                    raise KakaoAddressUnavailable("Kakao Local 앱 권한을 확인해 주세요.", reason="permission")
                if response.status_code >= 500:
                    raise KakaoAddressUnavailable("Kakao Local이 일시적인 서버 오류를 반환했습니다.", reason="upstream")
                response.raise_for_status()
                payload = response.json()
        except httpx.TimeoutException as exc:
            raise KakaoAddressUnavailable("Kakao Local 응답 시간이 초과됐습니다.", reason="timeout") from exc
        except KakaoAddressUnavailable:
            raise
        except (httpx.HTTPError, ValueError, TypeError) as exc:
            raise KakaoAddressUnavailable("카카오 주소 검색을 불러오지 못했습니다.", reason="upstream") from exc
