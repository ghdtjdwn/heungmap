...
2026-09-15 변경: 기본 수요 경로는 실제 D-30 지역 방문자-일 모델이며 `auto`에서 미지원 조건은
예측 불가로 표시합니다. 시군구 선택·기간 합계 범위·영향 요인과 기획→공개→방문객 동일 예측을
연결했습니다. 아래 모델·SHAP 제외 문구는 이전 범위입니다. [현재 모델 안내](MODEL_EVALUATION.md)를 따릅니다.
