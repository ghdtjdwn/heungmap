...
## 방문객 지역 필터 결과가 항상 0건인 경우

- 날짜: 2026-09-10
- 상황: 전체 축제 목록은 보이지만 방문객 화면에서 서울·인천 등 지역을 선택하면 결과가 0건이었습니다.
- 원인과 증거: 현재 KorService2 `searchFestival2`는 기존 `areaCode`가 아니라 법정동 시도 코드
  `lDongRegnCd`로 필터링합니다. 같은 기간에 `areaCode=1`은 0건, `lDongRegnCd=11`은 결과를 반환했고
  응답도 기존 `areacode` 대신 `lDongRegnCd`·`lDongSignguCd`를 제공했습니다.
- 해결: 공통 계약의 기존 광역 코드는 유지하고 TourAPI adapter에서 법정동 코드로 요청한 뒤 응답을 다시
  공통 코드로 변환합니다. 방문객 목록과 플래너 경쟁 행사 수는 같은 변환·전체 페이지 수집을 사용합니다.
- 검증: 실제 승인 키와 2026-09-01~12-09 범위에서 서울 55건, 인천 11건을 조회했고 모든 응답의
  `area_code`와 `legal_dong_code` 정규화를 확인했습니다. adapter·방문객·플래너 회귀 테스트도 함께 유지합니다.
