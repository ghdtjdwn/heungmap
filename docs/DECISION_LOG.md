...
## D24. 방문객 목록·지도 보기를 URL로 분리하고 조회 창을 다르게 적용

- 방문객 탐색은 `/visitor?view=list`의 목록 중심 화면과 `/visitor?view=map`의 큰 지도 중심 화면으로
  분리합니다. `view`가 없거나 잘못된 경우 목록 보기를 안전한 기본값으로 사용합니다.
- 검색어, 시작·종료일과 TourAPI 광역 지역 코드는 두 보기에서 같은 URL query로 유지해 새로고침,
  뒤로 가기와 공유 시 같은 탐색 조건을 복원합니다.
- 목록은 읽기와 페이지 이동을 위해 한 번에 20건, 지도는 공간 비교를 위해 같은 filter의 첫 100건을
  조회합니다. 두 화면 모두 기존 `GET /events`와 `EventSummary`를 사용하므로 공통 HTTP 계약은 변경하지
  않습니다.
- 지도는 조회된 좌표 전체에 맞춰 viewport를 조정하고, 좌표가 없거나 Kakao Maps를 불러오지 못해도
  행사 목록과 기존 `/visitor/{eventId}` 상세 경로를 계속 사용할 수 있게 합니다.
