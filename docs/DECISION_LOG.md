...
## D29. AI 재개 작업을 수요 모델 실험과 기획자 LLM 개선으로 분리

- 박지성은 공유 데이터·예측 계약 관리자를 맡고 Windows 환경에서 데이터·수요 모델 실험을 주도합니다.
  홍성주는 통합 검토자를 맡고 16GB Apple Silicon MacBook에서 기획자 LLM 추천·보고서 품질을 개선합니다.
- 두 작업은 최신 `main`과 기존 공통 Prediction 계약을 기준으로 병렬 진행합니다. 공통 schema나 endpoint
  변경이 필요하면 별도 shared PR과 상대 담당자 검토를 거칩니다.
- 기존 LightGBM은 시간 분할에서 baseline보다 16.23% 나빴으므로 새 실험도 채택 기준을 통과하기 전에는
  제품에 연결하지 않습니다. SHAP도 채택된 모델에만 적용합니다.
- LLM은 Prediction 수치를 읽기만 하며 새 숫자나 사실을 만들지 않습니다. model·LLM 실패 시 현재의
  규칙 mock, 입력 snapshot과 규칙 보고서를 유지합니다.
- 역할별 범위, 환경, 시작 명령, 결과물과 통합 순서는 `AI_WORK_SPLIT.md`에서 관리합니다.
