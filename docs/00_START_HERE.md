...
2026-09-15 역할 재조정: 박지성은 전체 UI·디자인과 두 사용자 흐름의 화면 품질을 담당하고,
홍성주는 AI 모델 개발에 집중합니다. 기능별 화면과 공통 예측 계약은 여전히 서로 검토합니다.
UI 작업 기준은 [UI 디자인 가이드](UI_DESIGN_GUIDE.md)를 따릅니다.

2026-09-18 제출 전 모델 운영화(재학습 절차·전향 평가·상태 점검·지역별 신뢰도·문체부 실제값)를 마쳤습니다.
현재 모델은 [모델 카드](MODEL_CARD.md), 실행·재현은 [모델 실행·평가](MODEL_EVALUATION.md)를 먼저 확인합니다.
아래 과거 전달·모델 보류 기록보다 D30·D31이 우선합니다.

현재 실행 가능한 통합 서비스와 인증·역할·행사 공개 흐름은 [SERVICE_INTEGRATION.md](SERVICE_INTEGRATION.md)를
먼저 확인합니다. 아래 문서가 아직 구현이 없다고 설명하는 부분과 초기 체크리스트는 작업 시작 당시 기록입니다.

이 문서는 두 팀원의 작업 진입점이다. 현재 `main`에는 실행 가능한 Next.js·FastAPI 서비스가 있다.
통합 PR #23은 2026-09-10 병합됐다. 병합 전 검증과 남은 제한 사항은
[세션 인계](NEXT_SESSION_COMMAND.md)를 확인한다. 아래 초기 작업 순서는 이력으로 유지하며,
이미 끝낸 데이터 게이트와 서비스 구현을 처음부터 반복하지 않는다.
...
| 기획자 서비스 담당 | 홍성주 |
| 사용자 서비스 담당 | 박지성 |
| 공유 데이터·예측 계약 관리자 (AI 수요 모델) | 홍성주 (2026-09-18 확정) |
| 통합 검토자 | 박지성 — 계약 관리자가 아닌 팀원. 상대 PR과 전체 demo를 검토 |
