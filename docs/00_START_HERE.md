...
| 기획자 서비스 담당 | 홍성주 |
| 사용자 서비스 담당 | 박지성 |
| 공유 데이터·예측 계약 관리자 | 박지성 — 데이터·수요 모델 실험과 공통 prediction 변경안 작성 |
| 통합 검토자 | 홍성주 — 공통 계약, 상대 PR과 전체 demo 검토 |
...
AI 작업을 재개하는 동안 박지성은 Windows에서 수요 모델·데이터 실험을, 홍성주는 Apple Silicon
MacBook에서 기획자 LLM 추천 품질을 담당한다. 두 작업의 경계와 병렬 진행 방법은
[`AI_WORK_SPLIT.md`](AI_WORK_SPLIT.md)를 따른다.
