...
2026-09-15 로컬 구현은 `regional_visit_demand`/`people` 범위를 두 서비스에서 사용합니다.
`GET /prediction/regions`는 기존 `RegionRef` 목록을 반환합니다. 값은 행사기간 시군구의 날짜별
방문자 수를 합한 방문자-일이며 특정 축제 관람객이 아닙니다. 계약 변경·가용성 한계는
[모델 실행·평가](MODEL_EVALUATION.md), 결정은 D30을 확인합니다. 모델 변경은 PR #31로 `main`에 병합됐고,
두 팀원의 표현·계약 재검토와 배포 검증은 남아 있습니다.
