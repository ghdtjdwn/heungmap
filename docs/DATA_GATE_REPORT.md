...
3. v1 L2 LightGBM, MAE용 L1 LightGBM, 기초지자체 범주와 TourAPI 행사 분류를 포함한 L1 후보를
   비교합니다.
...
### TourAPI 행사 상세·분류 feature 감사

`backend/scripts/audit_tourapi_detail_features.py`로 학습표에 존재하는 16개 광역 코드를 모두 포함한 고정
표본 60건을 조사했습니다. `detailCommon2`와 `detailIntro2` 응답률은 모두 100%였습니다. 원문 응답은
Git 제외 경로에만 보관하고 제공률·고유값 수만 기록합니다.

| 후보 필드 | 제공률 | 표본 고유값 수 | 한 값의 최대 비율 | 판정 |
| --- | ---: | ---: | ---: | --- |
| 대분류 | 100% | 1 | 100% | 변별력 없어 제외 |
| 중분류 | 100% | 3 | 68% | 후보 유지 |
| 소분류 | 100% | 11 | 22% | 후보 유지 |
| 상세 소개 | 100% | 60 | 2% | 구조화 방법 검증 전 보류 |
| 행사 장소 | 97% | 58 | 2% | 장소 유형 파생 규칙 검증 전 보류 |
| 운영시간 | 100% | 39 | 10% | 파싱 규칙 검증 전 보류 |
| 주최자 | 100% | 59 | 3% | 고 cardinality 원문 직접 입력 금지 |
| 요금 정보 | 100% | 24 | 60% | 무료·유료 파싱 검증 전 보류 |
| 보조 주최자 | 67% | 40 | 3% | 70% 제공률 기준 미달 |
| 소요시간 정보 | 2% | 1 | 100% | 제외 |

중·소분류는 상세 API뿐 아니라 이미 수집한 필수 `searchFestival2` 원본에도 존재합니다. 전체 최신 유효
축제 639건에서 두 필드의 제공률은 모두 100%이고 고유값은 각각 3개·18개였습니다. 추가 API 호출 없이
원본→정규화→학습표 경로를 보존해 새 후보에서 사용했습니다.

상세 소개·장소·주최자처럼 거의 모든 값이 서로 다른 원문을 작은 표본의 범주형 feature로 바로 넣으면
과적합 위험이 큽니다. 제공률을 확인한 것과 예측 가치가 확인된 것을 구분하며, 구조화 규칙과 별도 검증이
끝나기 전에는 모델 입력으로 채택하지 않습니다.

### 실제 결과

| 평가 | 지역 중앙값 baseline | 정규화 지역 평균 baseline | 선택 L1 달력·행사분류 LightGBM |
| --- | ---: | ---: | ---: |
| 훈련 구간 rolling 평균 MAE | 0.123001 | fold별 참고값 기록 | 0.114882 |
| rolling MAE 개선률 | 기준 | — | 6.60% |
| 최종 시간 분할 MAE | 0.077315 | **0.071073** | 0.083960 |
| 최종 시간 분할 RMSE | 0.104856 | **0.093801** | 0.113507 |
| 최종 MAE 개선률 | 기준 | 8.07% | **-8.59%** |
| 미관측 지역 5-fold 평균 MAE | 0.105840 | 해당 없음 | 0.099853 |
| 미관측 지역 MAE 개선률 | 기준 | — | 5.66% |

rolling 검증이 선택한 후보는 `lightgbm_l1_calendar_category`였지만 10% 개선 기준을 넘지 못했고 마지막 시간
...
| 실제 uplift 구간 | 행 수 | 지역 중앙값 MAE | L1 달력·행사분류 모델 MAE | 모델 개선률 | 모델 평균 오차 |
| --- | ---: | ---: | ---: | ---: | ---: |
| -5% 미만 감소 | 7 | 0.091478 | 0.136126 | -48.81% | +0.136126 |
| -5%~+5% 보합 | 30 | 0.038206 | 0.072970 | -90.99% | +0.072970 |
| +5% 초과 증가 | 66 | 0.093590 | 0.083423 | +10.86% | -0.055450 |

모델의 전체 평균 오차는 -0.005026으로 작지만 이는 구간별 반대 방향 오류가 상쇄된 결과입니다. 감소·보합
행사를 과대평가하고 증가 폭이 큰 행사를 과소평가하므로, 일정과 좌표만으로는 행사 자체의 수요 강도를
구분하지 못하고 중간 범위로 수축하는 패턴입니다.

월별로도 6월은 baseline보다 7.77%, 7월은 18.59% 악화됐습니다. 8월은 37.60% 개선됐지만 9건뿐이라
일반화 근거로 사용하지 않습니다. 광역 단위 결과도 일부 표본이 1~2건에 불과해 지역별 우열을 주장하지
않고 JSON에 행 수와 함께 진단값만 남깁니다.

다음 feature 검증 우선순위는 상세 감사에서 제공률을 통과한 정보를 안전하게 구조화하는 것과 공휴일·연휴
구성입니다. 특히 요금의 무료·유료 구분과 장소 유형은 파싱 정확도를 먼저 fixture로 검증합니다.
