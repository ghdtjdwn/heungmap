...
`/visitor?view=map`에서 같은 검색·날짜·지역 조건의 최대 100건을 큰 지도에서 비교한다. 두 보기의 상태는
URL query로 공유되며 `/visitor/{eventId}` 상세로 이어진다.
...
이미 3000번 개발 서버를 실행 중이라면 별도 E2E 서버 대신 다음처럼 그 서버를 재사용할 수 있다.
