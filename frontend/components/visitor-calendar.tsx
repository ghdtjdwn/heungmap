...
export function VisitorCalendar({ events, month, onMonth }: { events: EventSummary[]; month: string; onMonth: (month: string) => void }) {
  ...
  return <section className="panel calendar-panel" aria-label="행사 캘린더">
    ...
    <div className="calendar-grid">{["일", "월", "화", "수", "목", "금", "토"].map(day => <strong key={day}>{day}</strong>)}
      ...
      {Array.from({ length: days }, (_, i) => {
        ...
        return <div className="calendar-day" key={date}><time dateTime={date}>{day}</time>{events.filter(e => e.start_date <= date && e.end_date >= date).map(event => <Link key={event.event_id} href={"/visitor/" + event.event_id}>{event.title}</Link>)}</div>;
      })}
    </div>
