...
export function Welcome({ onboarding = false }: { onboarding?: boolean }) {
  ...
  return <main className="welcome-shell welcome-home" ref={rootRef}>
    <header className="welcome-nav"><BrandLink /><nav aria-label="소개 페이지"><a href="#flow">이용 방법</a><a href="#services">서비스</a></nav><span className="welcome-data-chip">KTO TourAPI 연동</span><button className="button primary" disabled={busy} onClick={() => void run(login)}>{session?.user ? "내 서비스로 이동" : "로그인"}</button></header>
    ...
    <section className="welcome-proof" aria-label="흥할지도 핵심 근거"><div data-reveal style={{ transitionDelay: "0ms" }}><span>01</span><strong>한국관광공사 TourAPI</strong><small>축제·관광정보의 기준 데이터</small></div><div data-reveal style={{ transitionDelay: "80ms" }}><span>02</span><strong>D-30 지역 방문수요</strong><small>시군구 방문자-일 예측 범위</small></div><div data-reveal style={{ transitionDelay: "160ms" }}><span>03</span><strong>기획에서 탐색까지</strong><small>공개한 행사를 방문객 화면으로 연결</small></div></section>
    <section id="flow" className="welcome-flow"><header data-reveal><h2>하나의 행사가<br />두 사람의 경험이 되기까지</h2></header><ol><li data-reveal style={{ transitionDelay: "0ms" }}><span>01</span><div><strong>조건을 정리하고</strong><p>지역·일정·규모와 아직 모르는 조건을 단계별로 입력합니다.</p></div></li><li data-reveal style={{ transitionDelay: "90ms" }}><span>02</span><div><strong>근거를 확인하고</strong><p>TourAPI 장소 정보와 지역 방문수요 범위, 해석 한계를 함께 봅니다.</p></div></li><li data-reveal style={{ transitionDelay: "180ms" }}><span>03</span><div><strong>세상에 공개합니다</strong><p>확정한 행사만 공개해 방문객의 목록·지도·달력으로 이어집니다.</p></div></li></ol></section>
